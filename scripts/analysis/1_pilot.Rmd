...
exp_name = "1_exit_22x40_ud"

parsed_trials <- read_csv("~/output/experiments/1_exit_22x40_ud/parsed_trials.csv") %>%
    mutate(resp_same = Response == "j",
         correct = !xor(base, resp_same))

exp_data <- read_csv("~/output/scenes/1_exit_22x40_ud.csv") %>%
  rename(scene = id)
