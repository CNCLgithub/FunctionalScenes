...
```{r}
model_raw <- read_csv("~/project/spaths/experiments/ccn_2023_exp/chain_summary_max_diff_50.csv")
...
pair_diff %>%
  with(lm(f_diff ~ diff_geo)) %>%
  summary()

pair_diff %>%
  ggplot(aes(x = diff_geo, y = f_diff, label = as.character(scene))) +
  geom_point() + 
