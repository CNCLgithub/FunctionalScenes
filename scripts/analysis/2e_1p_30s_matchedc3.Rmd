...
dprime_by_trial %>%
  ...
  summarise(dd.prime = diff(d.prime),
            ...
            prop.dd.prime = dd.prime / abs(first.d.prime)) %>%
  ungroup() %>%
  ggplot(aes(x = datt, y = prop.dd.prime, label = name)) + 
  geom_point() + 
...
dprime_by_trial %>%
  mutate(name = paste(scene, move)) %>%
  ggplot(aes(x = move_sense, y = d.prime, label = name,
             color = c3_3d)) + 
  geom_point() + 
  geom_text_repel(size=3.5) +
  geom_smooth(method = "lm") + 
