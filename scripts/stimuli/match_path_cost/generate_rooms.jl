...
function valid_pair(pair::Tuple, m::Move;
                    pc_diff_thresh::Float64 = 1.0,
                    pc_upper_bound::Float64 = 20.00
                    )::Bool
    ...
    # f > 0
    f > 0 &&
        abs(pc1 - pc2) < pc_diff_thresh &&
        max(pc1, pc2) < pc_upper_bound
end
...
function create_pair(door_conditions::Vector{GridRoom},
    ...
    d1 = expand(with_furn, 2) # door1 already added
    d2 = expand(add(with_furn, door_conditions[2]), 2)
    ...
    pc_1, _ = noisy_path(d1; path_params...)
    pc_2, _ = noisy_path(d2; path_params...)
...
function main()
    ...
    entrance = [8, 9]
    door_rows = [5, 12]
    inds = LinearIndices(rsteps)
    doors = inds[door_rows, rsteps[2]]
    ...
    t1 = GridRoom(rsteps, rbounds, entrance, [doors[1]])
    t2 = GridRoom(rsteps, rbounds, entrance, [doors[2]])
    templates = [t1, t2]
