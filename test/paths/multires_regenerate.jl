...
function coarsen_randomly(trace, params)
    new_trace = deepcopy(trace)
    for i = 1:18
        # number of steps to decrease resolution
        res_level = categorical([0.1, 0.6, 0.3]) - 1
        # selecting tracker addresses
        for j = 1:res_level
            lvl = new_trace[:trackers => i => :level]
            state = new_trace[:trackers => i => :state]
            coarsened = FunctionalScenes.coarsen_state(params, state, lvl - 1)
            cm = choicemap((:trackers => i => :level, lvl - 1),
                           (:trackers => i => :state, coarsened))
            (new_trace, _, _, _) = update(new_trace, cm)
        end
    end
    return new_trace
end
...
ws = ones(steps(r))
r = last(furniture_chain(20, r, ws))
r = expand(r, 2)
params = ModelParams(;
                        gt = r,
                        dims = (6, 6),
...
FunctionalScenes.gt_init_constraints!(constraints, params, r)
# all trackers at full-res
trace, ll = generate(model, (params,), constraints)

FunctionalScenes.viz_global_state(trace)
...
new_trace = coarsen_randomly(trace, params)
println("\n\n\nNEW TRACE")
